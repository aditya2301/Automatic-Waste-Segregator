...
def cloudResponse(img):
  s = socket.socket()
  file_name=datetime.datetime.now().strftime("Image-%d-%m-%y_%H-%M-%S.jpeg")
  cv2.imwrite(file_name,img)

  s.connect(('IP addr of the server', 80))
  f=open(file_name,"rb")
  data=f.read()
  f.close()
  length=str(len(data))
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        try:
           ...
           if cv2.contourArea(c)>300 and len(contours)<=5:
            if counter==0:
                print("Going to sleep for 0.2 second")
                time.sleep(0.2)
                counter=1
                continue
            else:
                ...
                #identification_thread=threading.Thread(target=identification)
           ...
           #while direction_flag==0:
            #continue
           direction_flag=cloudRespone(image)
           servoControl.flap(directionFlag)
           #direction_flag=0
